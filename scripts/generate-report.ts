...
import { buildReport, GLOSSARY, type WbrReport } from '../src/lib/wbr/report';
...
function buildXlsx(rep: WbrReport, path: string) {
  ...
  if (rep.highlights.length) add('Highlights', [['Key highlights — what the numbers say'], ...rep.highlights.map((h) => [h])]);
  if (rep.reviewQueue.length) add('Review Queue', [['Topic', 'Assigned bucket'], ...rep.reviewQueue.map((q) => [q.topic, q.category])]);
  add('Glossary', [['Term', 'Definition'], ...GLOSSARY.map((g) => [g.term, g.def])]);
...
function buildHtml(rep: WbrReport): string {
  ...
  const highlights = rep.highlights.length
    ? `<section class="hlbox"><h2>Key Highlights — what the numbers say</h2><ul class="hl">${rep.highlights.map((h) => `<li>${h}</li>`).join('')}</ul></section>`
    : '';
  ...
  *{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;color:var(--ink);margin:0;background:#f6f6fa}
  ...
  .hlbox{background:#f4f3ff;border:1px solid #ddd9fb;border-radius:12px;padding:14px 18px}.hlbox h2{color:#4f46e5}
  ul.hl{margin:6px 0 0;padding-left:18px}ul.hl li{font-size:13px;margin-bottom:7px}
  ...
  ${kpis}${highlights}${sections.join('')}
  <section><h2>What these terms mean</h2><div class="tw"><table><tbody>${
    GLOSSARY.map((g) => `<tr><td style="font-weight:600;white-space:normal">${g.term}</td><td style="white-space:normal;color:var(--muted)">${g.def}</td></tr>`).join('')
