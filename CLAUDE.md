...
- **Chart labels stay short** (under ~30 characters). `blogCharts.ts`'s bar chart puts the label on its own line above the bar so a long one no longer collides with it, but SVG `<text>` still never wraps, so a very long label just runs off the edge instead of breaking layout. Short labels are still the actual fix.
- **Never hand off a post with a chart, or any new visual component, without actually rendering it first.** A real bug shipped this way: chart labels longer than the old fixed-width column got silently painted over by the bar itself, for several posts in a row, because the mockup was only ever tested with short placeholder labels, never the real long ones later posts actually used. Before saying a post (or a template change) is done, render it locally and look at the actual output, the same way any other feature in this repo gets verified before shipping.
